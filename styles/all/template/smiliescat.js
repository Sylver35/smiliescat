(function($){  // Avoid conflicts with other libraries
	...
	cats.loadPagination = function(total){
		...
		if((totalPages > 1) && (total > cats.perPage)){
			...
			for(var i = startFor; i < endCnt; i++){
				items.push(cats.cE('a','nb-'+(i - 1) * cats.perPage,(i === onPage) ? 'pagin_red' : 'pointer',cats.page+i,i,(i !== onPage) ? function(){cats.changePage(this.id.replace('nb-',''));} : false,'button'));
	...
	cats.cE = function(sort,id,className,title,innerHTML,onClick,name){
	...
	cats.cp = function(){
		return cats.cE('span',false,'page-sep',false,', ',false,false);
	};

	cats.closeAction = function(){
		$('#cat-title, #smileys-list, #smilies-pagin, #cats-list').html('').hide();
		$('#smilies-pagin-div, #u-close').hide();
		$('#u-open').show();
	};
